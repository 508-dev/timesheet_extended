...
function validate(dialog, frm, overlap_allowed) {
	...
	if (!frm.doc.employee) {
		problems.push(__("Set the Employee on the timesheet first -- billing rates depend on it."));
	}
...
function parse_csv(text) {
	...
	lines.forEach(function (line, index) {
		...
		const hours = parseFloat(raw_hours);
 ...
 *
 * Seconds are optional, since spreadsheets routinely drop a trailing :00.
 */
 ...
 * time and only after a round trip. Catch them here instead, against both
 * the incoming rows and the rows already in the grid.
 */
...
function append_rows(frm, args, rows, rates) {
	...
	// recalculates all of it server-side on save regardless.
