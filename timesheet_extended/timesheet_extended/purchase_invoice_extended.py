	...
	try:
		...
		for timesheet_name in timesheets:
			try:
				...
				frappe.log_error(f"Error updating timesheet {timesheet_name} from purchase invoice {doc.name}: {str(e)}")
	except Exception as e:
	...
	try:
		...
		for timesheet_name in timesheets:
			try:
				...
				frappe.log_error(f"Error unlinking timesheet {timesheet_name} from purchase invoice {doc.name}: {str(e)}")
	except Exception as e:
		frappe.log_error(f"Error fetching timesheets for purchase invoice {doc.name}: {str(e)}")
