...
@frappe.whitelist()
def get_projectwise_timesheet_data_with_employee(
    project=None, parent=None, from_time=None, to_time=None, supplier=None
):
    """
    ...
    if supplier:
        condition += "AND emp.supplier = %(supplier)s "
    ...
    query = f"""
        SELECT
                ...
                emp.supplier as employee_supplier
...
"""
    ...
    filters = {"project": project, "parent": parent,
               "from_time": from_time, "to_time": to_time, "supplier": supplier}

    return frappe.db.sql(query, filters, as_dict=1)


@frappe.whitelist()
def get_projectwise_timesheet_data_for_purchase_invoice(
        project=None, parent=None, from_time=None, to_time=None, supplier=None
):
    """
...
def make_purchase_invoice(source_name, item_code=None, supplier=None, currency=None):
    ...
    if item_code:
        target.append("items", {"item_code": item_code,
                      "qty": hours, "rate": costing_rate})
    ...
    timesheet.purchase_invoice_status = "Created"
    # Will be set when Purchase Invoice is submitted
